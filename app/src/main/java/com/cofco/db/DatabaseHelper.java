...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	private static final String DATABASE_NAME = "cocfo.db";
	// any time you make changes to your database objects, you may have to increase the database version
	private static final int DATABASE_VERSION = 1;

	// the DAO object we use to access the SimpleData table
	private Dao<SimpleData, Integer> simpleDao = null;
	private Dao<CreativeItem, Integer> creativeDao = null;
	 ...
	 * value.
	 */
	public Dao<CreativeItem, Integer> getCreativeDataDao() throws SQLException {
		if (creativeDao == null) {
			creativeDao = getDao(CreativeItem.class);
		}
		return creativeDao;
	}
	...
	private void onCreate() {
		try {
			...
			TableUtils.createTable(connectionSource, CreativeItem.class);
			// here we try inserting data in the on-create as a test
			Dao<CreativeItem, Integer> dao = getCreativeDataDao();
			long millis = System.currentTimeMillis();
			CreativeItem item = new CreativeItem();
			dao.create(item);
