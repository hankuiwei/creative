...
public class CreativeDetailActivity extends BaseActivity {
	...
	protected void findViewById() {
		...
		mLayoutVote = (RelativeLayout) this
				.findViewById(R.id.layout_detail_vote);
		mLayoutReplay = (RelativeLayout) this
				.findViewById(R.id.layout_detail_replay);
		mLayoutPraise = (RelativeLayout) this
				.findViewById(R.id.layout_detail_praise);
		mLayoutFavorite = (RelativeLayout) this
				.findViewById(R.id.layout_detail_favorite);
	...
	protected void loadViewLayout() {
		...
		mScreenHeight = Util.getScreenHeight(CreativeDetailActivity.this);
	...
	protected void processLogic() {
		...
		if (id == 1) {
			mCreativeImage.setImageResource(R.drawable.icon1);
		} else if (id == 2) {
			mCreativeImage.setImageResource(R.drawable.icon2);
		} else if (id == 3) {
			mCreativeImage.setImageResource(R.drawable.icon3);
		} else if (id == 4) {
			mCreativeImage.setImageResource(R.drawable.icon4);
		} else if (id == 5) {
			mCreativeImage.setImageResource(R.drawable.icon5);
		}
		String[] photos = creativeItem.getPhoto();
		if (photos!=null && photos.length > 0) {
			mPictureLayout.setVisibility(View.VISIBLE);
			for (int i = 0; i < photos.length; i++) {
				if(photos[i].substring(0,1).equals(".")){
					photos[i]=photos[i].substring(1, photos[i].length());
				}
				//url[i] = Constant.IMAGE_BASE_URL + photos[i];
				url[i] = getString(R.string.app_host) + photos[i];
			}
			if (photos.length == 1) {
				mPicture1.setVisibility(View.VISIBLE);
				showImageView(url[0], mPicture1);
			} else if (photos.length == 2) {
				mPicture1.setVisibility(View.VISIBLE);
				mPicture2.setVisibility(View.VISIBLE);
				showImageView(url[0], mPicture1);
				showImageView(url[1], mPicture2);
			} else if (photos.length == 3) {
				mPicture1.setVisibility(View.VISIBLE);
				mPicture2.setVisibility(View.VISIBLE);
				mPicture3.setVisibility(View.VISIBLE);
				showImageView(url[0], mPicture1);
				showImageView(url[1], mPicture2);
		...
		mCreativeTime.setText(Util.getDate(creativeItem.getCreate_time()));
		getInnovationDetail();
	}
	...
	public void onClick(View v) {
		...
		case VOTE:
			// 进行投票
			if (!NetUtil.hasNetwork(CreativeDetailActivity.this)) {
				CommonUtil.showInfoTost(CreativeDetailActivity.this,
						getString(R.string.net_errors));
				return;
			}
			if (voteStatus==1) {
				Util.showToast(CreativeDetailActivity.this,
						getString(R.string.voted_text), -(mScreenHeight / 4));
				return;
			}else if(voteStatus==2){
				Util.showToast(CreativeDetailActivity.this, "此创意不在投票期间");
				return;
			} else {
				Util.showToast(CreativeDetailActivity.this,
				...
				vo.mContext = CreativeDetailActivity.this;
		...
		case REPLAY:
			...
			intent.setClass(CreativeDetailActivity.this, CommentActivity.class);
		...
		case PRAISE:
			// 进行攒
			if(!NetUtil.hasNetwork(CreativeDetailActivity.this)){
				CommonUtil.showInfoTost(CreativeDetailActivity.this, getString(R.string.net_errors));
				return;
			}
			if (praiseStatus) {
				Util.showToast(CreativeDetailActivity.this,
				...
				String praiseNum = mPraiseNum.getText().toString().trim();
				String num = String.valueOf(Integer.valueOf(praiseNum) + 1);
		...
		case FAVORITE:
			...
			if (collectStatus) {
				Util.showToast(CreativeDetailActivity.this,
						getString(R.string.favorited_text),
						-(mScreenHeight / 4));
				return;
			} else {
				Util.showToast(CreativeDetailActivity.this,
				...
				if (paramObject != null && paramObject != null) {
					try {
						...
						if (status.equals("200")) {
							...
							Util.showToast(CreativeDetailActivity.this, msg,
									-(mScreenHeight / 4));
							return;
						}
					} catch (JSONException e) {
...
//				imageView.setVisibility(View.VISIBLE);
				...
				imageView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						Bitmap resizeBitmap = Bitmap.createBitmap(loadedImage,
								0, 0, bitmapWidth, bitmapHeight, matrix, true);
